...
- [x] **T2.4** 3D 模型查看器（react-three-fiber）— 支持 .glb 加载 + 程序化 fallback
...
- [x] **T5.3** Vercel 导入 + 自动部署 ✅

---

## P2 内容完善 ✅ (2026-03-02)

- [x] Product 类型新增 `name` / `description` 字段
- [x] **products.json** 扩展至 8 个 SKU（3 品类：cutting-board / serving-tray / utensil）
- [x] **ProductCard** 真实图片 + next/image + fallback
- [x] **ImageGallery** 新建客户端组件（点击切换主图）
- [x] **产品详情页** 全面改造（ImageGallery + description + product.name）
- [x] **ModelViewer** 支持 .glb 加载（useGLTF + 程序化 fallback）
- [x] **i18n** 新增品类翻译键（servingTrays / utensils）
- [x] **PRODUCT_GUIDE.md** 产品内容管理操作指南
...
### 优先级 P2 — 内容替换
- [ ] 替换 `public/images/products/` 中的真实产品照片
- [ ] 替换 `public/models/` 中的真实 .glb 模型文件
- [ ] 补充 About Us 页面内容
- [ ] 更新 products.json 中的产品规格为真实数据
